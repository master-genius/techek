...
$app->group('', function() use ($app) {
    ...
    $app->get('[/]', function($req, $res) {
        $page = $this->View->page('techgeek/index-hljs2.html', $this->Config);
...
$app->group('/w', function() use ($app) {
    ...
    $app->get('[/]', function($req, $res) {
        $page = $this->View->page('techgeek/home.html', $this->Config);
        return ApiRet::raw($res, $page);
    });

    $app->get('/rs/add', function($req, $res) {
        $page = $this->View->page('techgeek/rsadd.html', $this->Config);
        return ApiRet::raw($res, $page);
    });

    $app->get('/rs/edit/{id}', function($req, $res) {
        $page = $this->View->page('techgeek/rsedit.html', $this->Config);
        return ApiRet::raw($res, $page);
    });

    $app->get('/rs/wlist', function($req, $res) {
        $page = $this->View->page('techgeek/rslist.html', $this->Config);
        return ApiRet::raw($res, $page);
    });

    $app->get('/media/list', function($req, $res) {
        $page = $this->View->page('techgeek/media.html', $this->Config);
        return ApiRet::raw($res, $page);
    });

    $app->get('/lecture', function($req, $res) {
        $page = $this->View->page('techgeek/lecture.html', $this->Config);
        return ApiRet::raw($res, $page);
    });

    $app->get('/rs/mdadd', function($req, $res) {
        $page = $this->View->page('techgeek/mdadd.html', $this->Config);
        return ApiRet::raw($res, $page);
    });
    
    $app->get('/rs/mdedit/{id}', function($req, $res) {
        $page = $this->View->page('techgeek/mdedit.html', $this->Config);
        return ApiRet::raw($res, $page);
    });
